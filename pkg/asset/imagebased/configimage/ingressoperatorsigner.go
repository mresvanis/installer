...
func (a *IngressOperatorSignerCertKey) Generate(dependencies asset.Parents) error {
	...
	return a.SelfSignedCertKey.Generate(cfg, "ingress-operator-signer")
}
...
func (a *IngressOperatorCABundle) Generate(deps asset.Parents) error {
	var certs []tls.CertInterface
	for _, asset := range a.Dependencies() {
		deps.Get(asset)
		certs = append(certs, asset.(tls.CertInterface))
	}
	return a.CertBundle.Generate("ingress-operator-ca-bundle", certs...)
}
