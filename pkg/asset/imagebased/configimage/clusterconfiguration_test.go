...
func TestClusterConfiguration_Generate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expectedConfig: clusterConfiguration().proxy((*lca.Proxy)(proxy())).build().Config,
		},
		{
			...
			expectedConfig: clusterConfiguration().additionalTrustBundle(lca.AdditionalTrustBundle{
		...
		},
		{
			...
			expectedConfig: clusterConfiguration().
				proxy((*lca.Proxy)(proxy())).
				additionalTrustBundle(lca.AdditionalTrustBundle{
					UserCaBundle: testCert,
				}).
				build().Config,
		},
...
func (ccb *ClusterConfigurationBuilder) proxy(proxy *lca.Proxy) *ClusterConfigurationBuilder {
	ccb.ClusterConfiguration.Config.Proxy = proxy
	return ccb
}

func (ccb *ClusterConfigurationBuilder) additionalTrustBundle(additionalTrustBundle lca.AdditionalTrustBundle) *ClusterConfigurationBuilder {
...
func clusterConfiguration() *ClusterConfigurationBuilder {
	...
	cc.Config.KubeconfigCryptoRetention = lca.KubeConfigCryptoRetention{
		...
		IngresssCrypto: lca.IngresssCrypto{
			IngressCA: (lca.PEM)(string(ingressCertKey().SelfSignedCertKey.Key())),
		},
	}
