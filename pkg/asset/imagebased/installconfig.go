...
func (a *OptionalInstallConfig) validateSNOConfiguration(installConfig *types.InstallConfig) field.ErrorList {
	...
	if machineNetworksCount != 1 {
		...
		allErrs = append(allErrs, field.Invalid(fieldPath, installConfig.Networking.MachineNetwork, "Only 1 MachineNetwork is allowed for image-based Single Node OpenShift (SNO) cluster"))
	}
